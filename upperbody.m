...
%             msg.Y=y;
            ...
            stringvar=['D:\snap\upper\cie',num2str(frameCount)];
...
%     rosnode list
    ...
    timesave=[timesave;exctime];
    %count=count+1;
    xsave=[xsave;x];
    ysave=[ysave;y];
    figure(2);
    plot(xsave(1:frameCount));
    hold on
    plot(ysave(1:frameCount));
    xlim([0 frameCount]);
    ylim([-30 30]);
    title('respon deteksi');
    xlabel('posisi');
    ylabel('waktu');
    legend('x position','y position');
    drawnow;
    figure(3);
    plot(timesave(1:frameCount));
    xlim([0 frameCount]);
    ylim([0 10]);
    title('respon deteksi');
    xlabel('frame');
